...
var folderBase = $"{configuration.Version}\\{deploymentTarget}\\{topology}";
    // Clean deployment folders
     string[] folders = { $"\\{folderBase}\\assets\\HeroHelixPOC", $"\\{folderBase}\\assets\\HeroHelixPOCCD", $"\\{folderBase}\\Website", $"\\{folderBase}\\assets\\HeroHelixPOC_xConnect", $"\\{folderBase}\\assets\\Data Exchange Framework\\WDPWorkFolder", $"\\{folderBase}\\assets\\Data Exchange Framework CD\\WDPWorkFolder" };
...
Task("Publish-Foundation-Projects").Does(() => {
    ...
    if (!deployLocal){
        destination = $"{deploymentRootPath}\\Website\\HeroHelixPOC";
    }
...
Task("Publish-Project-Projects").Does(() => {
    ...
    var HeroHelixPOC = $"{configuration.ProjectSrcFolder}\\HeroHelixPOC";
    var HeroHelixPOCBasic = $"{configuration.ProjectSrcFolder}\\HeroHelixPOCBasic";
    ...
    PublishProjects(HeroHelixPOC, destination);
    PublishProjects(HeroHelixPOCBasic, destination);
});
...
Task("Publish-xConnect-Project").Does(() => {
   ...
   if (!deployLocal){
        destination = $"{deploymentRootPath}\\Website\\HeroHelixPOC_xConnect";
    }
...
Task("Publish-Transforms").Does(() => {
    ...
    if (!deployLocal){
        destination = $@"{deploymentRootPath}\Website\HeroHelixPOC\temp\transforms";
    }
...
.IsDependentOn("Generate-HeroHelixPOCUpdatePackages")
.IsDependentOn("Generate-HeroHelixPOCWDP");

Task("Publish-YML").Does(() => {

	var serializationFilesFilter = $@"{configuration.ProjectFolder}\**\*.yml";
    var destination = $@"{deploymentRootPath}\Website\HeroHelixPOC\App_Data";
...
Task("Generate-HeroHelixPOCUpdatePackages").Does(() => {
	StartPowershellFile ($"{configuration.ProjectFolder}\\Azure\\HelperScripts\\Generate-HeroHelixPOCUpdatePackages.ps1", args =>
        {
            args.AppendQuoted($"{configuration.ProjectFolder}\\cake-config.json");
        });
		});

Task("Generate-HeroHelixPOCWDP").Does(() => {
	StartPowershellFile ($"{configuration.ProjectFolder}\\Azure\\HelperScripts\\Generate-HeroHelixPOCWDP.ps1", args =>
        {
            args.AppendQuoted($"{configuration.ProjectFolder}\\cake-config.json");
        });
		});
...
Task("Publish-Post-Steps").Does(() => {
    ...
    var destination = $@"{deploymentRootPath}\Website\HeroHelixPOC\App_Data\poststeps";
...
Task("Prepare-Transform-Files").Does(()=>{
    ...
    var destination = $@"{deploymentRootPath}\Website\HeroHelixPOC\";  
    var layers = new string[] { configuration.FoundationSrcFolder, configuration.FeatureSrcFolder, configuration.ProjectSrcFolder};
